...
def data_migration(apps, schema_editor):
    ...
    # Chain Link before is "Assign file UUIDs and checksums: Set file permissions".
    # Its UUID is 25b8ddff-4074-4803-a0dc-bbb3acd48a97.
    # As above, so below. The following would give us DSpace and Maildir chain
    # links, which we don't want::
    #
    #     MicroServiceChainLink.objects.filter(
    #         currenttask__description='Set file permissions',
    #         microservicegroup='Assign file UUIDs and checksums'
    #     )
    assign_uuids_file_perm_chain_link_uuid = '25b8ddff-4074-4803-a0dc-bbb3acd48a97'
    ...
    StandardTaskConfig.objects.create(
        ...
        arguments=('"%SIPDirectory%" "%SIPUUID%" --include-dirs'
    ...
    MicroServiceChainLinkExitCode.objects\
        .filter(nextmicroservicechainlink=assign_uuids_objects_chain_link)\
        .update(nextmicroservicechainlink=dir_uuid_choice_chain_link)
    MicroServiceChainLink.objects\
        .filter(defaultnextchainlink=assign_uuids_objects_chain_link)\
        .exclude(id=dir_uuid_chain_link_uuid)\
        .update(defaultnextchainlink=dir_uuid_choice_chain_link)
    ...
    MicroServiceChainLinkExitCode.objects.filter(
        microservicechainlink=assign_uuids_file_perm_chain_link).update(
            nextmicroservicechainlink=dir_uuid_choice_chain_link)
    assign_uuids_file_perm_chain_link.defaultnextchainlink = (
        dir_uuid_choice_chain_link)
    ...
    operations = [
        ...
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('originallocation', main.models.BlobTextField(
                    db_column=b'originalLocation')),
                ('currentlocation', main.models.BlobTextField(
                    null=True, db_column=b'currentLocation')),
                ('enteredsystem', models.DateTimeField(
                    auto_now_add=True, db_column=b'enteredSystem')),
                ('sip', models.ForeignKey(
                    db_column=b'sipUUID', blank=True, to='main.SIP',
                    null=True)),
                ('transfer', models.ForeignKey(
                    db_column=b'transferUUID', blank=True, to='main.Transfer',
                    null=True)),
            ],
            options={
